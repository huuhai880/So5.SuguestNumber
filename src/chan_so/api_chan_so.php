    ...
    if(isset($so_chan)){
        ...
        for ($i = 0; $i < count($so_chan); $i++) {
            ...
            for ($j = 0; $j < count($lst_so); $j++) {
                ...
                $sql_select = "SELECT 1 FROM `max_price` WHERE `number_limit` = '$so' AND `vung_mien` = '$dai_chan' AND `tai_khoan_tao` = '$ten_tai_khoan';";

                if ($sql_connector->get_query_result($sql_select)->num_rows == 0){

                    $sql = "INSERT IGNORE INTO `max_price` (`number_limit`, `vung_mien`, `tai_khoan_tao`) VALUES ('$so', '$dai_chan','$ten_tai_khoan')";
