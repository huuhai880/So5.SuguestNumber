...
class tin_mb
{
    ...
    {
        if ($sql_connector === null)
            return chi_tiet_tin_mb::lay_chi_tiet_cua_tin($this->id);
        return chi_tiet_tin_mb::lay_chi_tiet_cua_tin($this->id, $sql_connector);
    }
    ...
    {
        $tins = tin_mb::LayTinChuaCoKetQua($ten_tai_khoan, $loai_tai_khoan);
        if (sizeof($tins) == 0) {
            //echo 'Không có tin cần soi!';
            return;
        }
        foreach ($tins as $tin) {
            $ds_chi_tiet = chi_tiet_tin_mb::lay_chi_tiet_cua_tin($tin->id);
            $result = tin_mb::CapNhatKetQuaTin($tin, $ds_chi_tiet);
        }
    ...
    public static function LayTinChuaCoKetQua(string $ten_tai_khoan, string $loai_tai_khoan): array
    {
        ...
        return tin_mb::doc_tin_tu_db($sql);
    }


    public static function xoa_tat_ca_tin_theo_tai_khoan(string $ten_tai_khoan, sql_connector $sql_connector = null)
    {
        $tin_list = tin_mb::doc_tin_tu_db("SELECT * FROM tin WHERE tai_khoan_danh = '$ten_tai_khoan'");
    ...
    public static function CapNhatThongKeChoTin(tin_mb $tin, array $ds_chi_tiet): array
    {

        $result = array();
        $da_co_ket_qua = tin_mb::DaCoKetQua($tin);
        ...
        //Biến thống kê
        $thong_ke = array(
            '2c-dd' => new tin_thongke_mb('2c-dd'),
            '2c-b' => new tin_thongke_mb('2c-b'),
            '3c' => new tin_thongke_mb('3c'),
            '4c' => new tin_thongke_mb('4c'),
            'dat' => new tin_thongke_mb('dat'),
            'dax' => new tin_thongke_mb('dax'),
        );
        ...
        $html_thong_ke = tin_thongke_mb::toHTMLFormArray($thong_ke);

        $tin = tin_mb::CapNhatThongKeVaoTin($thong_ke, $tin);
    ...
    public static function CapNhatKetQuaTin(tin_mb $tin, array $ds_chi_tiet): array
    {
        $result = array();
        $da_co_ket_qua = tin_mb::DaCoKetQua($tin);
    ...
    public static function GhiTinVaChiTiet(tin_mb $tin, array $ds_chi_tiet)
    {
    ...
    public static function DaCoKetQua(tin_mb $tin): bool
    {
    ...
    public static function CapNhatThongKeVaoTin(array $thong_ke, tin_mb $tin): tin_mb
    {
...
class chi_tiet_tin_mb
{
    ...
    public function toHTML(): string
    {
        ...
        return '<tr> 
                    <td>' . $this->dai . '</td>
				    <td>' . chi_tiet_tin_mb::ChuanHoaSo($this->so) . '</td>
				    <td>' . $this->kieu . '</td>
				    <td>' . $this->diem . '</td>
    ...
    public function toHTML_web(): string
    {
        return ' <tr role="row" class="">
                    ...
                    <td aria-colindex="2" role="cell" class="">' . chi_tiet_tin_mb::ChuanHoaSo($this->so) . '</td>
